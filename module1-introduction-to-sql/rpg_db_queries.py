...
query9 = 'SELECT(SELECT COUNT(item_id) FROM armory_item)-(SELECT COUNT(item_ptr_id) FROM armory_weapon)' # 137 non-weapons
...
query12 = 'SELECT AVG(items_per_character) FROM (SELECT charactercreator_character.character_id, COUNT(charactercreator_character_inventory.item_id) AS items_per_character FROM charactercreator_character LEFT JOIN charactercreator_character_inventory ON charactercreator_character.character_id = charactercreator_character_inventory.character_id GROUP BY charactercreator_character_inventory.character_id)'
# average of 2.97 items per character
...
#assume: include characters that don't have weapons
#we want two columns: character_id and weapon_count
'SELECT AVG(weapon_count) as avg_weapons_per_char FROM (SELECT c.character_id ,count(distinct w.item_ptr_id) as weapon_count FROM charactercreator_character c LEFT JOIN charactercreator_character_inventory inv ON c.character_id = inv.character_id LEFT JOIN armory_weapon w ON inv.item_id = w.item_ptr_id GROUP BY c.character_id) subq)'
  #,c."name"
  #inv.*
  #.w*
